...
convert_image() {
    ...
    if [[ -f "$file" ]] && [[ -r "$file" ]] && [[ "$file" =~ \.(JPG|jpg|JPEG|jpeg|PNG|png|webp|gif|bmp|tiff|HEIF|heif|HEIC|heic)$ ]]; then
...
find "$source_dir" -type f -print0 | while IFS= read -r -d '' file; do
    ...
    if [[ "$file" =~ \.(JPG|jpg|JPEG|jpeg|PNG|png|webp|gif|bmp|tiff|mp4|MOV|mov|avi|mkv|flv|mpeg|mpg|HEIF|heif|HEIC|heic)$ ]]; then
        # Convert the file to AVIF or Matroshka based on its type
        if [[ "$file" =~ \.(JPG|jpg|JPEG|jpeg|PNG|png|webp|gif|bmp|tiff|HEIF|heif|HEIC|heic)$ ]]; then
